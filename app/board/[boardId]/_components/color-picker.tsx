...
export const ColorPicker = ({ onChange }: ColorPickerProps) => {
  return (
    <div className="flex flex-wrap gap-2 items-center max-w-[164px] pr-2 mr-2 border-neutral-200">
      ...
      <ColorButton color={{ r: 39, g: 142, b: 236 }} onClick={onChange} />
...
const ColorButton = ({ onClick, color }: ColorButtonProps) => {
  return (
    <button
      className="w-8 h-8 flex items-center justify-center hover:opacity-75 transitionbg-red-500"
      onClick={() => onClick(color)}
    >
      <div
        className="h-8 w-8 rounded-md border border-neutral-200"
        style={{ background: parseColorToCss(color) }}
      ></div>
    </button>
  );
};
